...
const EventDetails = () => {
  ...
  useEffect(() => {
    const fetchEventDetails = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`https://campusconnect2-0.onrender.com/api/events/${id}`);
        setEvent(response.data);
        
        // Check if user has already applied
        if (isAuthenticated && user?.userType === 'student') {
          try {
            const applicationResponse = await axios.get(`https://campusconnect2-0.onrender.com/api/applications/student/my-applications`, {
