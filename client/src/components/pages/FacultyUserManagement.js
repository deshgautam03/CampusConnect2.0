...
const FacultyUserManagement = () => {
  ...
  const load = async () => {
    ...
    try {
      const [s, c] = await Promise.all([
        axios.get('https://campusconnect2-0.onrender.com/api/users/students'),
        axios.get('https://campusconnect2-0.onrender.com/api/users/coordinators')
      ]);
      setStudents(s.data);
  ...
  const setStatus = async (id, isActive) => {
    try {
      await axios.put(`https://campusconnect2-0.onrender.com/api/users/${id}/status`, { isActive });
      await load();
